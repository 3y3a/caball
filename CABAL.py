...
token = NjkzMzc0NTA0NTU4Mzk1NDM1.XsEkog.k4qv8nyKyLopMJ0Js7PoLa5JRt8
...
async def Правила(ctx):
    await ctx.send (f"""```Выбор категории:
    SCP
    Классы_людей```
    """)
...
@cabal.command(pass_context= True)
async def SCP_all (ctx):
    r_all = open ("Общие_SCP.txt")
    for line in r_all:
        await ctx.send (line)
    r_all.close

@cabal.command(pass_context= True)
async def SCP_049 (ctx):
    r_049 = open ("049.txt")
    for line in r_049:
        await ctx.send (line)
    r_049.close
...
@cabal.command(pass_context= True)
async def Human_all (ctx):
    rh_all = open ("Общие_Люди.txt")
    for line in rh_all:
        await ctx.send (line)
    rh_all.close
